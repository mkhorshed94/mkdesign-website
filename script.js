...
function spatialScroll(){
  ...
  document.querySelectorAll('.project').forEach((card,i)=>{
    ...
    const side=i%2?1:-1;
    const focus=1-Math.min(1,Math.abs(p));
    frame.style.transform=`translateZ(45px) scale(${.91+focus*.09}) translateY(${p*34}px)`;
    image.style.transform=`scale(${1.16-focus*.06}) translate3d(0,${p*-34}px,0)`;
    image.style.filter=`brightness(${.68+focus*.32}) blur(${(1-focus)*2.5}px)`;
    card.classList.toggle('scene-active',Math.abs(p)<.38);
  });
  const cards=[...document.querySelectorAll('.project')];
  let closest=0,best=Infinity;
  cards.forEach((card,i)=>{const d=Math.abs(card.getBoundingClientRect().top-innerHeight*.1);if(d<best){best=d;closest=i}});
  const counter=document.getElementById('sceneCurrent');if(counter)counter.textContent=String(closest+1).padStart(2,'0');
